...
try {
  ...
  await page.goto(C + '/', 4000);
  ok(/9 tools/i.test(await site('Harness Lab')) === false, 'switched to Harness Lab');
  const d1 = await ask('I want to learn to design evals for coding agents. What does this site offer and what does it need to know about me?', 'describe_learning_offer');
  console.log(d1.slice(-600));
  await site('Vault');
  const d2 = await ask('Create the readiness assertion the Harness Lab needs for its three requirements. Its origin is ' + H + ' and the purpose is personalize-agent-evals-path.', 'create_readiness_assertion');
